    ...
    public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (servico.Id_Usuario != _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier).Value)
                return Result.Fail("E503", "somente quem criou a Servico pode remover");

            await _repository.RemoverServicoAsync(servico);
            return Result.Ok();
        }
        catch (Exception ex)
        {
            return Result.Fail("E599", $"Houve um erro ao remover servico {ex.InnerException.Message}");
        }
