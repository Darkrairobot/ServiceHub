...
public class Handler : IRequestHandler<Command, Result>
{
    ...
    public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (empresa.Id_Usuario != _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier).Value)
                return Result.Fail("E403", "somente quem criou a Empresa pode Remover");

            await _repository.DeletarEmpresaAsync(empresa);
            return Result.Ok();
        }
        catch (Exception ex)
        {
            return Result.Fail("E399", $"Houve um erro ao remover empresa {ex.Message}");
        }
